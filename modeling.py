...
def pack_inputs(inputs):
    ...
    outputs = [tf.constant(0, shape=[], dtype=tf.int32)if x is None else x for x in inputs]
...
def unpack_inputs(inputs):
    ...
    """
    inputs = tf.nest.flatten(inputs)
    outputs = [None if is_special_none_tensor(x) else x for x in inputs]
    if len(outputs) == 1:
        return outputs[0]
    return tuple(outputs)
...
def get_activation(identifier):
    ...
    if isinstance(identifier, six.string_types):
        ...
        if identifier in ACTIVATION_FUNC:
            return tf.keras.activations.get(ACTIVATION_FUNC[identifier])
...
class EmbeddingPostprocessor(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.initializer = get_initializer(self.initializer_range) if not initializer else initializer
...
class Dense3D(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        ret = tf.einsum("abcd,cde->abe", inputs, kernel) if self.output_projection else tf.einsum("abc,cde->abde", inputs, kernel)
        if self.use_bias:
            ret += bias
...
class Dense2DProjection(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        dtype = tf.as_dtype(self.dtype or tf.keras.backend.floatx())
        if not (dtype.is_floating or dtype.is_complex):
            raise TypeError("Unable to build `Dense2DProjection` layer with non-floating point (and non-complex) "
                            "dtype %s" % (dtype,))
        input_shape = tf.TensorShape(input_shape)
        if tf.compat.dimension_value(input_shape[-1]) is None:
...
class TransformerBlock(tf.keras.layers.Layer):
    ...
    def build(self, unused_input_shapes):
        ...
        self.output_dense = Dense2DProjection(
            ...
            kernel_initializer=get_initializer(self.initializer_range), name="output")
